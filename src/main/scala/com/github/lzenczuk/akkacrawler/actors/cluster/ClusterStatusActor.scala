...
import com.github.lzenczuk.akkacrawler.models.cluster.{Cluster, ClusterInitEvent, NodeStatus, NodeUpdateEvent, _}
...
class ClusterState private[cluster] {
  ...
  def process(event:MemberEvent):List[ClusterEvent] = {
    event match {
      ...
      case e =>
        println(s"unknown event $e")
        List()
    }
  }

  def process(event:LeaderChanged):List[ClusterEvent] = {
  ...
  private def updateNodeStatus(address: Address, status: NodeStatus):List[ClusterEvent] = {
